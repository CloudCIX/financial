...
class Permissions:
    ...
    def list(request: Request, address_id: int, span) -> Optional[Http403]:
        """
        The request to list the data for a Sales by Country is valid if:
        ...
        if not request.user.global_active:
            return Http403(error_code='financial_sales_by_country_list_201')
        ...
        if response.status_code != 200 or response.json()['content']['member']['id'] != request.user.member['id']:
            return Http403(error_code='financial_sales_by_country_list_202')
