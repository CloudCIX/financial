...
class Permissions:
    ...
    def list(request: Request, address_id: int, span) -> Optional[Http403]:
        """
        The request to list the data for a Trial Balance statement is valid if:
        - The requesting User is listing data from their own Address
        - The requesting User is Global Active and is listing data from anther Address in their Member
        """
        ...
        if not request.user.global_active:
            return Http403(error_code='financial_trial_balance_list_201')
        ...
        if response.status_code != 200 or response.json()['content']['member']['id'] != request.user.member['id']:
            return Http403(error_code='financial_trial_balance_list_202')
