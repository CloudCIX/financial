...
class PurchasesByCountryListController(ControllerBase, TransactionMixin):
    """
    ...
    def validate_address_id(self, address_id: Optional[int]) -> Optional[str]:
        """
        description: |
            The address_id to generate the Purchase By Country from.  If not sent it will default to what the
            requesting User has permission for.
        type: string
        required: false
        """
        if address_id is not None:
            try:
                address_id = self._validate_integer(address_id, 'financial_purchases_by_country_list_101')
    ...
    def validate_start_date(self, start_date: Optional[str]) -> Optional[str]:
        ...
        if start_date is not None:
            try:
                start_date = self._validate_date(start_date, 'financial_purchases_by_country_list_102')
            except FinancialException as e:
                return e.args[0]
    ...
    def validate_finish_date(self, finish_date: Optional[str]) -> Optional[str]:
        ...
        if finish_date is not None:
            try:
                finish = self._validate_date(finish_date, 'financial_purchases_by_country_list_103')
            except FinancialException as e:
                return e.args[0]
        else:
