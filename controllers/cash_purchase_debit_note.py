...
class CashPurchaseDebitNoteCreateController(ControllerBase):
    """
    Validate User data to create a new Nominal Ledger record for a Cash Purchase Debit Note transaction
    """
    ...
    def validate_address1_deliver_to(self, address: Optional[str]) -> Optional[str]:
        """
        description: The first line of the geographic address to deliver the items of the Cash Purchase Debit Note to
        type: string
        """
        if address is None:
            address = ''
        ...
        if len(address) == 0:
            return 'financial_cash_purchase_debit_note_create_101'
        if len(address) > self.get_field('address1_deliver_to').max_length:
            return 'financial_cash_purchase_debit_note_create_102'
        self.cleaned_data['address1_deliver_to'] = address
        return None

    def validate_address2_deliver_to(self, address: Optional[str]) -> Optional[str]:
        """
        description: The second line of the geographic address to deliver the items of the Cash Purchase Debit Note to
        type: string
        required: false
        """
        if address is None:
        ...
        if len(address) > self.get_field('address2_deliver_to').max_length:
            return 'financial_cash_purchase_debit_note_create_103'
        self.cleaned_data['address2_deliver_to'] = address
        return None

    def validate_address3_deliver_to(self, address: Optional[str]) -> Optional[str]:
        """
        description: The third line of the geographic address to deliver the items of the Cash Purchase Debit Note to
        type: string
        required: false
        """
        if address is None:
        ...
        if len(address) > self.get_field('address3_deliver_to').max_length:
            return 'financial_cash_purchase_debit_note_create_104'
        self.cleaned_data['address3_deliver_to'] = address
        return None

    def validate_city_deliver_to(self, city_deliver_to: Optional[str]) -> Optional[str]:
        """
        description: The city where the Cash Purchase Debit Note's items will be delivered to
        type: string
        """
        ...
        if len(city_deliver_to) == 0:
            return 'financial_cash_purchase_debit_note_create_105'
        if len(city_deliver_to) > self.get_field('city_deliver_to').max_length:
            return 'financial_cash_purchase_debit_note_create_106'
    ...
    def validate_contra_address_id(self, contra_address_id: Optional[int]) -> Optional[str]:
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_107'
        ...
        if response.status_code != 200:
            return 'financial_cash_purchase_debit_note_create_108'
    ...
    def validate_contra_contact_id(self, contra_contact_id: Optional[int]) -> Optional[str]:
        """
        description: |
            The id of a User in the Contra Address to contact with inquiries about the Cash Purchase Debit Note
        type: integer
        required: false
        """
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_109'
        ...
        if response.status_code != 200 or response.json()['content']['address']['id'] != contra_address_id:
            return 'financial_cash_purchase_debit_note_create_110'
    ...
    def validate_country_id_deliver_to(self, country_id_deliver_to: Optional[int]) -> Optional[str]:
        """
        description: The id of the Country where the Cash Purchase Debit Note items will be delivered to
        type: integer
        """
        try:
            country_id_deliver_to = int(cast(int, country_id_deliver_to))
        except (TypeError, ValueError):
            return 'financial_cash_purchase_debit_note_create_111'
        ...
        if response.status_code != 200:
            return 'financial_cash_purchase_debit_note_create_112'
    ...
    def validate_external_reference(self, external_reference: Optional[str]) -> Optional[str]:
        """
        description: The identifier for this Cash Purchase Debit Note in the Contra Address' referencing scheme
        type: string
        required: false
        """
        ...
        if len(external_reference) > self.get_field('external_reference').max_length:
            return 'financial_cash_purchase_debit_note_create_113'
        self.cleaned_data['external_reference'] = external_reference
        return None

    def validate_lines(self, lines: Optional[List[CREDIT]]) -> Optional[str]:
        """
        description: A collection of the amounts to credit from each Nominal Account for this Cash Purchase Debit Note
        type: array
        items:
            type: object
            properties:
        ...
        if not isinstance(lines, list):
            return 'financial_cash_purchase_debit_note_create_114'

        account_numbers: Set[int] = set()
        tax_rate_ids: Set[int] = set()
        for line in lines:
            if not isinstance(line, dict):
                return 'financial_cash_purchase_debit_note_create_115'

            try:
                description = cast(str, line['description'])
            except KeyError:
                return 'financial_cash_purchase_debit_note_create_116'
            if len(description) > NominalLedgerDebit._meta.get_field('description').max_length:
                return 'financial_cash_purchase_debit_note_create_145'

            try:
                line['exchange_rate'] = Decimal(str(line.get('exchange_rate', 1)))
            except InvalidOperation:
                return 'financial_cash_purchase_debit_note_create_117'

            try:
                number = int(cast(int, line['number']))
                if number not in range(8000):
                    return 'financial_cash_purchase_debit_note_create_118'
                line['number'] = number
                account_numbers.add(number)
            except (KeyError, TypeError, ValueError):
                return 'financial_cash_purchase_debit_note_create_119'

            try:
                line['quantity'] = Decimal(str(line['quantity']))
            except (InvalidOperation, KeyError):
                return 'financial_cash_purchase_debit_note_create_120'

            if 'tax_amount' in line:
                try:
                    line['tax_amount'] = Decimal(str(line['tax_amount']))
                except (InvalidOperation):
                    return 'financial_cash_purchase_debit_note_create_121'
            ...
            try:
                ...
                return 'financial_cash_purchase_debit_note_create_122'

            try:
                line['unit_price'] = Decimal(str(line['unit_price']))
            except (InvalidOperation, KeyError):
                return 'financial_cash_purchase_debit_note_create_123'
        ...
        if len(accounts) != len(account_numbers):
            return 'financial_cash_purchase_debit_note_create_124'
        for a in accounts:
            if not a.global_nominal_account.valid_purchases_account and \
                ...
                return 'financial_cash_purchase_debit_note_create_125'
        ...
        if len(tax_rates) != len(tax_rate_ids):
            return 'financial_cash_purchase_debit_note_create_126'
        # Put the records into a dictionary for easy access
        ...
        for line in lines:
            ...
            if 'tax_amount' in line:
                if (partial_tax - tax_error) <= cast(Decimal, line['tax_amount']) <= (partial_tax + tax_error):
                    ...
                    return 'financial_cash_purchase_debit_note_create_127'
            else:
        ...
        if gross_amount <= Decimal('0'):
            return 'financial_cash_purchase_debit_note_create_128'
    ...
    def validate_name_deliver_to(self, name_deliver_to: Optional[str]) -> Optional[str]:
        """
        description: The name of the company to deliver the Cash Purchase Debit Note's items to
        type: string
        """
    ...
    def validate_narrative(self, narrative: Optional[str]) -> Optional[str]:
        ...
        if len(narrative) > self.get_field('narrative').max_length:
            return 'financial_cash_purchase_debit_note_create_131'
    ...
    def validate_payment_method_id(self, payment_method_id: Optional[int]) -> Optional[str]:
        """
        description: |
            How the requesting User intends to receive the Debit Note payment. This will determine what Account will be
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_132'
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_133'
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_134'
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_135'

        if address_account.global_nominal_account.nominal_account_number not in VALID_ACCOUNT_RANGE:
            return 'financial_cash_purchase_debit_note_create_136'
    ...
    def validate_postcode_deliver_to(self, postcode_deliver_to: Optional[str]) -> Optional[str]:
        """
        description: The postcode that the Purchase Debit Note's items will be delivered to
        type: string
        required: false
        """
        ...
        if len(postcode_deliver_to) > self.get_field('postcode_deliver_to').max_length:
            return 'financial_cash_purchase_debit_note_create_137'
    ...
    def validate_report_template_id(self, report_template_id: Optional[int]) -> Optional[str]:
        """
        description: The id of the Report Template to use when printing the Cash Purchase Debit Note
        type: integer
        """
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_138'
        ...
        if response.status_code != 200:
            return 'financial_cash_purchase_debit_note_create_139'
        if response.json()['content']['idTransactionType'] != 10001:
            return 'financial_cash_purchase_debit_note_create_140'
    ...
    def validate_subdivision_id_deliver_to(self, subdivision_id_deliver_to: Optional[int]) -> Optional[str]:
        """
        description: The id of the Subdivision where the Purchase Debit Note's items will be delivered to
        type: integer
        required: false
        """
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_create_141'
        ...
        if response.status_code != 200:
            return 'financial_cash_purchase_debit_note_create_142'
    ...
    def validate_transaction_date(self, date: Optional[str]) -> Optional[str]:
        """
        description: The date that the Purchase Debit Note was issued
        type: string
        """
        try:
            transaction_date = datetime.strptime(str(date).split('T')[0], '%Y-%m-%d').date()
        except (TypeError, ValueError):
            return 'financial_cash_purchase_debit_note_create_143'
        obj = NominalLedger.period_end.filter(
            address_id=self.request.user.address['id'],
            transaction_date__gte=transaction_date,
        )
        if obj.exists():
            return 'financial_cash_purchase_debit_note_create_144'
        self.cleaned_data['transaction_date'] = transaction_date
        return None


class CashPurchaseDebitNoteUpdateController(ControllerBase):
    """
    Validate User data to update an existing Nominal Ledger record for a Cash Purchase Debit Note transaction
    """
    ...
    def validate_external_reference(self, external_reference: Optional[str]) -> Optional[str]:
        ...
        external_reference = str(external_reference).strip()
        if len(external_reference) > self.get_field('external_reference').max_length:
            return 'financial_cash_purchase_debit_note_update_101'
        self.cleaned_data['external_reference'] = external_reference
        return None


class CashPurchaseDebitNoteContraCreateController(ControllerBase):
    """
    Validate User data to create a new Nominal Ledger record for a Cash Purchase Debit Note transaction
    """
    ...
    def validate_payment_method_id(self, payment_method_id: Optional[int]) -> Optional[str]:
        """
        description: How the User will receive the value of the Debit Note
        type: integer
        """
        try:
            payment_method_id = int(cast(int, payment_method_id))
        except (TypeError, ValueError):
            return 'financial_cash_purchase_debit_note_contra_create_101'
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_contra_create_102'
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_contra_create_103'
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_contra_create_104'

        if address_account.global_nominal_account.nominal_account_number not in VALID_ACCOUNT_RANGE:
            return 'financial_cash_purchase_debit_note_contra_create_105'
    ...
    def validate_report_template_id(self, report_template_id: Optional[int]) -> Optional[str]:
        ...
        try:
            ...
            return 'financial_cash_purchase_debit_note_contra_create_106'
        ...
        if response.status_code != 200:
            return 'financial_cash_purchase_debit_note_contra_create_107'
        if response.json()['content']['idTransactionType'] != 10001:
            return 'financial_cash_purchase_debit_note_contra_create_108'
        self.cleaned_data['report_template_id'] = report_template_id
        return None

    def validate_transaction_date(self, date: Optional[str]) -> Optional[str]:
        """
        description: The date that the Cash Purchase Debit Note was created
        type: string
        """
        try:
            transaction_date = datetime.strptime(str(date).split('T')[0], '%Y-%m-%d').date()
        except (TypeError, ValueError):
            return 'financial_cash_purchase_debit_note_contra_create_109'
        obj = NominalLedger.period_end.filter(
            address_id=self.request.user.address['id'],
            transaction_date__gte=transaction_date,
        )
        if obj.exists():
            return 'financial_cash_purchase_debit_note_contra_create_110'
        self.cleaned_data['transaction_date'] = transaction_date
        return None

    def validate_tsn(self, tsn: Optional[int]) -> Optional[str]:
        """
        description: The Transaction Sequence Number of a Case Sale Credit Note in the specified Contra Address
        type: integer
        """
        try:
            tsn = int(cast(int, tsn))
        except (TypeError, ValueError):
            return 'financial_cash_purchase_debit_note_contra_create_111'

        try:
            credit_note = NominalLedger.cash_sale_credit_notes.get(
                tsn=tsn,
                address_id=self.address_id,
                contra_address_id=self.request.user.address['id'],
            )
        except NominalLedger.DoesNotExist:
            return 'financial_cash_purchase_debit_note_contra_create_112'

        if credit_note.contra_nominal_ledger is not None:
            return 'financial_cash_purchase_debit_note_contra_create_113'

        self.cleaned_data['contra_nominal_ledger'] = credit_note
        return None
    ...
    def validate_lines(self, lines: Optional[List[CREDIT]]) -> Optional[str]:
        ...
        """
        if not isinstance(lines, list):
            return 'financial_cash_purchase_debit_note_contra_create_114'

        if 'contra_nominal_ledger' not in self.cleaned_data:
            return None
        credit_note = self.cleaned_data['contra_nominal_ledger']

        # Make sure each line in the transaction is accounted for. One extra debit entry is made for VAT when a Cash
        # Sale Credit Note gets created
        debits = credit_note.debits.all()
        if len(lines) != len(debits) - 1:
            return 'financial_cash_purchase_debit_note_contra_create_115'

        account_numbers: Set[int] = set()
        tax_rate_ids: Set[int] = set()
        for line in lines:
            if not isinstance(line, dict):
                return 'financial_cash_purchase_debit_note_contra_create_116'

            try:
                description = cast(str, line['description'])
            except KeyError:
                return 'financial_cash_purchase_debit_note_contra_create_117'
            if len(description) > NominalLedgerDebit._meta.get_field('description').max_length:
                return 'financial_cash_purchase_debit_note_contra_create_128'

            try:
                line['exchange_rate'] = Decimal(str(line.get('exchange_rate', 1)))
            except InvalidOperation:
                return 'financial_cash_purchase_debit_note_contra_create_118'

            try:
                number = int(cast(int, line['number']))
                if number not in range(8000):
                    return 'financial_cash_purchase_debit_note_contra_create_119'
                line['number'] = number
                account_numbers.add(number)
            except (KeyError, TypeError, ValueError):
                return 'financial_cash_purchase_debit_note_contra_create_120'

            try:
                line['quantity'] = Decimal(str(line['quantity']))
            except (InvalidOperation, KeyError):
                return 'financial_cash_purchase_debit_note_contra_create_121'
            ...
            try:
                ...
                return 'financial_cash_purchase_debit_note_contra_create_122'

            try:
                line['unit_price'] = Decimal(str(line['unit_price']))
            except (InvalidOperation, KeyError):
                return 'financial_cash_purchase_debit_note_contra_create_123'
        ...
        if len(accounts) != len(account_numbers):
            return 'financial_cash_purchase_debit_note_contra_create_124'
        for a in accounts:
            if not a.global_nominal_account.valid_purchases_account:
                return 'financial_cash_purchase_debit_note_contra_create_125'

        # Make sure all the Tax Rates exits
        tax_rates = TaxRate.objects.filter(
            address_id=self.request.user.address['id'],
            id__in=tax_rate_ids,
        )
        if len(tax_rates) != len(tax_rate_ids):
            return 'financial_cash_purchase_debit_note_contra_create_126'
        # Put the Tax Rate records into a dictionary for easy access
        tax_rates = {obj.id: obj for obj in tax_rates}

        # Now make sure each line matches one of the credits from the Account Sale Invoice
        gross_amount = Decimal('0')
        tax_amount = Decimal('0')
        results: Deque[CREDIT] = deque()
        for d in debits:
            if d.nominal_account_number == reserved.VAT_CONTROL_ACCOUNT:
                # We can't take the VAT line directly from the contra. The VAT must be calculated from the lines in the
            ...
            for line in lines:
                ...
                if line['description'] == d.description and line['unit_price'] == d.unit_price and \
                        line['quantity'] == d.quantity and tax_rate.percent == d.tax_percent:
                    match_found = True
            ...
            if not match_found:
                return 'financial_cash_purchase_debit_note_contra_create_127'
        ...
        # Add in one debit for the Account specified by the `payment_method_id`
        self.cleaned_data['debit'] = {
